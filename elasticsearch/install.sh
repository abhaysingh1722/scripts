...
function configureAndInstall() {
	...
	printf -- '****************Downloading Elasticsearch********************** \n' | tee -a "$LOG_FILE"
	git clone -q https://github.com/elastic/elasticsearch
	cd elasticsearch
	git checkout v6.4.2 | tee -a "$LOG_FILE"
	sleep 3
...
"ubuntu-16.04" | "ubuntu-18.04")
	...
	sudo apt-get update
	sudo apt-get install -y tar wget unzip curl maven git make automake autoconf libtool patch libx11-dev libxt-dev pkg-config texinfo locales-all ant hostname
	prepare
	configureAndInstall
	startService
	installClient
...
"rhel-7.3" | "rhel-7.4" | "rhel-7.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" | tee -a "$LOG_FILE"
	sudo yum install -y unzip curl which git gcc-c++ make automake autoconf libtool libstdc++-static tar wget patch words libXt-devel libX11-devel texinfo ant ant-junit.noarch hostname
	prepare
	configureAndInstall
	startService
	installClient
...
"sles-12.3" | "sles-12.2")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" | tee -a "$LOG_FILE"
	sudo zypper --non-interactive install tar wget unzip curl which git words gcc-c++ patch libtool automake autoconf ccache xorg-x11-proto-devel xorg-x11-devel alsa-devel cups-devel libffi48-devel libstdc++6-locale glibc-locale libstdc++-devel libXt-devel libX11-devel texinfo ant ant-junit.noarch make net-tools
	prepare
	configureAndInstall
	startService
	installClient
...
"rhel-7.3" | "rhel-7.4" | "rhel-7.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" | tee -a "$LOG_FILE"
	sudo yum install -y unzip curl which git gcc-c++ make automake autoconf libtool libstdc++-static tar wget patch libXt-devel libX11-devel texinfo ant ant-junit.noarch hostname
	prepare
	configureAndInstall
	startService
	installClient
...
"sles-12.3" | "sles-15")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" | tee -a "$LOG_FILE"
	sudo zypper --non-interactive install tar wget unzip curl which git gcc-c++ patch libtool automake autoconf ccache xorg-x11-proto-devel xorg-x11-devel alsa-devel cups-devel libstdc++6-locale glibc-locale libstdc++-devel libXt-devel libX11-devel texinfo ant ant-junit.noarch make net-tools
	prepare
	configureAndInstall
	startService
	installClient
